...
| HTTPS 代理 | 支持 SNI passthrough、静态证书终止和 ACME DNS-01 托管证书终止。 |
| 证书管理 | 控制通道 TLS 材料可自动生成；HTTPS 托管证书支持 Cloudflare DNS-01、签发、续期、状态查询和热加载。 |
...
| 没有静态证书，但存在 active 托管证书 | 状态为 `valid` 或 `expiring_soon` 且证书/私钥文件存在时，执行 TLS 终止。 |
| 没有完整静态证书，也没有 active 托管证书 | HTTPS 代理保持 SNI passthrough：服务端只用 SNI 选路，不解密公网 TLS，把加密字节流转发到客户端本地 HTTPS 目标。 |
