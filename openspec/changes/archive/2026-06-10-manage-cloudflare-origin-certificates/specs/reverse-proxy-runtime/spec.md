...
系统 MUST 在 HTTP 类反向代理转发到客户端本地 HTTP 目标前，默认把目标可见的请求 `Host` 改写为配置的目标地址，并在安全可解析时改写已有 `Origin`。该行为 MUST 适用于 HTTP 反向代理和 HTTPS TLS 终止后的 HTTP 转发；HTTPS proxy 没有可服务证书时 MUST 拒绝或关闭连接，而不是声明任何 HTTP header rewrite 行为。
...
HTTPS 反向代理运行时 MUST 只使用通过 active material 健康检查且未被确认 provider-side 不可用的托管证书完成公网 TLS 终止。

#### Scenario: Usable managed certificate terminates TLS
- **WHEN** TLS ClientHello SNI 主机匹配已启用 HTTPS 代理，且该代理主机存在通过健康检查并未被确认 provider-side 不可用的托管 active certificate material
- **THEN** 服务端使用该 active certificate material 终止公网 TLS，并把解密后的 HTTP 请求转发到客户端本地 HTTP 目标

#### Scenario: Failed renewal or rotation still serves valid active certificate
- **WHEN** TLS ClientHello SNI 主机匹配已启用 HTTPS 代理，该代理最近一次托管证书续期或轮换失败，但 active certificate material 仍通过健康检查且未被确认 provider-side 不可用
...
#### Scenario: Provider-disabled managed material is not used
- **WHEN** TLS ClientHello SNI 主机匹配已启用 HTTPS 代理，但该代理主机的托管 active certificate material 已被 provider sync 确认为撤销或 provider-side 不可用
- **THEN** 服务端 MUST NOT 使用该托管证书完成 TLS 终止，并把该代理标记为证书失效或需要配置状态
