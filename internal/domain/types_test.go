...
func TestRewriteWebProxyPath(t *testing.T) {
	if got := RewriteWebProxyPath("/api/users", "/api", true, "/v1"); got != "/v1/users" {
		t.Fatalf("unexpected rewritten path: %q", got)
	}
	if got := RewriteWebProxyPath("/api", "/api", true, "/v1"); got != "/v1" {
		t.Fatalf("unexpected prefix-only path: %q", got)
	}
}

func TestProxyValidateRejectsReservedWebPathPrefix(t *testing.T) {
	proxy := Proxy{ID: "p1", UserID: "u1", ClientID: "c1", Name: "web", Type: ProxyWeb, Status: ProxyEnabled, DomainID: "d1", PathPrefix: "/.well-known/goginx/activate", UpstreamPathPrefix: "/", TargetHost: "127.0.0.1", TargetPort: 8080}
	if err := proxy.Validate(); err == nil {
		t.Fatal("expected reserved web path prefix error")
	}
}
