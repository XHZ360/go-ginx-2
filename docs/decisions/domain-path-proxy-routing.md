...
- 同一 Domain 下可保存多个规范化后相同的 `PathPrefix`，但任一时刻最多一个对应 Proxy 可处于启用态；创建、更新或启用时只与已启用 Proxy 校验冲突，运行时按最长路径段前缀选择 Proxy。
...
- 对同一 `(domain_id, normalized_path_prefix)`，任一时刻最多一个 Proxy 可启用；禁用 Proxy 不参与冲突校验。`/api` 匹配 `/api` 和 `/api/users`，不匹配 `/apix`。
